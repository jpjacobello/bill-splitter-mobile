...
// Venmo: official app icon (white V on blue tile). No usable single-path monochrome
// mark exists, so we use the real raster logo. `color` is ignored here.
export function VenmoLogo({ size = 16 }: { size?: number; color?: string }) {
  return (
    <Image
      source={require('../assets/images/venmo-icon.jpg')}
      style={{ width: size, height: size, borderRadius: size * 0.22 }}
    />
  );
}

export function CashAppLogo({ size = 16, color = '#00C244' }: { size?: number; color?: string }) {
  return (
